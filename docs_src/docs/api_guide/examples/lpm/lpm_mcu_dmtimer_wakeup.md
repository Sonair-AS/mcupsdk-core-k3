# LPM DMTimer Wakeup {#EXAMPLES_LPM_MCU_DMTIMER_WAKEUP}
...
- This example provides support for MCU only low power mode support on the MCU core (MCU M4F). To run MCU-only low power mode, refer \ref EXAMPLES_LPM_MCU_DMTIMER_WAKEUP_MCU_ONLY_LPM
...
- This example provides support for MCU only low power mode support on the MCU core (MCU R5F). To run MCU-only low power mode, refer \ref EXAMPLES_LPM_MCU_DMTIMER_WAKEUP_MCU_ONLY_LPM
...
- We can modify the timeout value by modifying the `Tick Period` field in Timer Module in the example.syscfg for this example. Refer \ref SYSCONFIG_INTRO_PAGE
...
## MCU only LPM {#EXAMPLES_LPM_MCU_DMTIMER_WAKEUP_MCU_ONLY_LPM}
